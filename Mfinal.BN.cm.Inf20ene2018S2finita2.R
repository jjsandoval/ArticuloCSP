#install.packages("runjags"); install.packages("rjags"); install.packages("mcmcplots"); install.packages("ggplot2")
require(runjags)
require(rjags)
require(reshape2)
library(ggplot2)
library(mcmcplots)
################################# MODELO DE BINOMIAL NEGATIVO ################################
...
#
################################## Recategorizar educacion ####################################
ttasas0913.cr.inf.cm<-ttasas0913.cr.inf.cm[order(ttasas0913.cr.inf.cm$ESC),] # ordenar por ESC
#
x<-matrix(NA,50,2) # Matriz de union de freacuencias y poblacion 1 y 2
for(i in 1:50){
x[i,1]<-ttasas0913.cr.inf.cm[i,6]+ttasas0913.cr.inf.cm[i+50,6]
x[i,2]<-ttasas0913.cr.inf.cm[i,7]+ttasas0913.cr.inf.cm[i+50,7]
}
ttasas0913.cr.inf.cm<-subset(ttasas0913.cr.inf.cm, ESC>=2)
ttasas0913.cr.inf.cm$FREC[ttasas0913.cr.inf.cm$ESC==2]<-x[,1]
ttasas0913.cr.inf.cm$Pob.cr[ttasas0913.cr.inf.cm$ESC==2]<-x[,2]
#
ttasas0913.cr.inf.cm$ESC[ttasas0913.cr.inf.cm$ESC==2]<-1 
ttasas0913.cr.inf.cm$ESC[ttasas0913.cr.inf.cm$ESC==3]<-2
ttasas0913.cr.inf.cm$ESC[ttasas0913.cr.inf.cm$ESC==4]<-3
ttasas0913.cr.inf.cm$ESC[ttasas0913.cr.inf.cm$ESC==5]<-4 

###################### MODELO BAYESIANO BINOMIAL NEGATIVO EN JAGS ######################
set.seed(2^31-1) # Semilla primo de mersene
nbinomial.infb<-
"model{
# N observations
for (i in 1:N){
...
ES[1]~dnorm(1.02, 15.67)
ES[2]~dnorm(0.84, 141.7)
ES[3]~dnorm(0.37, 141.7)
# Hyperparametros de la priori
r ~ dgamma(0.01,0.01)
a<-1/r
# Calculo de los valores ajustados (sin tasas aun)
for (i in 1:N){Est[i]<-(lambda[i]/E[i])*100000} # valores ajustados
# Residual Pearson
...
#
inits <- list(list(b0=rnorm(1), SEX=c(NA,rnorm(1)), ED=c(NA, rnorm(4)), ES=c(rnorm(3), NA),
                Ano=c(NA, rnorm(4)), r=runif(1,0,50)),
              list(b0=rnorm(1), SEX=c(NA,rnorm(1)), ED=c(NA, rnorm(4)), ES=c(rnorm(3), NA),
                   Ano=c(NA, rnorm(4)), r=runif(1,0,50)),
              list(b0=rnorm(1), SEX=c(NA,rnorm(1)), ED=c(NA, rnorm(4)), ES=c(rnorm(3), NA),
                   Ano=c(NA, rnorm(4)), r=runif(1,0,50)))
#
nbin.inf.cm<- run.jags(model=nbinomial.infb, 
                       ...
                       data = datosbn.inf.cm, inits = inits,
                       n.chains = 3, sample = 1000, 
                       burnin = 1000, adapt=1000, thin = 10)
Est.bn<-extend.jags(nbin.inf.cm, add.monitor="Est", sample = 1000, thin = 10,
...
#
###################......RESULTADOS....###################################
sink("result.bn.20enero2018.inf.cm.txt") # guardar resultados
print(nbin.inf.cm)
dic.bn.cm
nbin.inf.cm$summary
nbin.inf.cm$HPD
...
tab.desvio2.bn.inf.cm<-cbind("Año"=datosbn.inf.cm$ANO, "Edad"=datosbn.inf.cm$ED5, "Sex"=datosbn.inf.cm$SEXO, 
                          "Esc"=datosbn.inf.cm$ESC, "Freq"=datosbn.inf.cm$NUM, "Pob"=datosbn.inf.cm$E, 
                          round(desvio2.bn.HPD[,1:3],1))
tab.pred.bn.inf.cm2<-data.frame(tab.pred.bn.inf.cm)
tab.pred.bn.inf.cm2$ts<-round(tab.pred.bn.inf.cm2$Freq/tab.pred.bn.inf.cm2$Pob*100000,2)
tab.desvio2.bn.inf.cm<-data.frame(tab.desvio2.bn.inf.cm)
write.csv(tab.pred.bn.inf.cm2, "tab.pred.bn.inf.cm3.csv")
options(scipen=999)
# Guardar graficos
plot(tab.pred.bn.inf.cm2$ts,tab.pred.bn.inf.cm2$Median, 
     main = "Model NB con Missing Imputado", ylim=c(0,15000), xlim=c(0,15000))
abline(lsfit(tab.pred.bn.inf.cm2$ts,tab.pred.bn.inf.cm2$Median), col="red", lty=2)
...
plot(Est.bn, plot.type = c("trace", "density", "histogram", "autocorr"), layout=c(4,4))
dev.off()
...
op <- par(mfrow = c(1, 2))
plot(tab.pred.bn.inf.cm2$Median, tab.desvio2.bn.inf.cm$Median, 
